...
#[test]
fn hotpath_maker_focus_swap() {
    #[cfg(feature = "hotpath")]
    let hotpath_guard = {
        if env::var_os("HOTPATH_METRICS_SERVER_OFF").is_none() {
            env::set_var("HOTPATH_METRICS_SERVER_OFF", "1");
        }
        hotpath::HotpathGuardBuilder::new("hotpath_maker_focus_swap").build()
    };

    // CI narrows report output by setting HOTPATH_FOCUS=coinswap::maker.
